...
<div id="sidebar" class="w-64 bg-gray-800 text-white fixed h-full transition-all pt-16">
    <div class="logo-container px-4">
        <img style="background-color: white; border-radius: 50%;" src="{{ asset('images/psu-logo.png') }}" alt="PSU Logo"
            loading="lazy" class="w-8 h-8 object-contain shadow-lg">
    ...
    <ul class="px-4">
        <li class="py-2">
            ...
            {{ request()->routeIs('super-admin.dashboard') ? 'bg-blue-700 px-2 rounded-sm' : '' }}">
        ...
        <li class="py-2">
            <a href="{{ route('super-admin.users') }}"
                class="text-lg text-white hover:px-2 hover:text-white hover:bg-blue-700 hover:rounded-sm transition-colors flex items-center
            {{ request()->routeIs('super-admin.users') ? 'bg-blue-700 px-2 rounded-sm' : '' }}">
...
<div class="fixed inset-0 bg-black bg-opacity-50 z-50 justify-center items-center hidden peer-checked:flex">
    <div class="bg-white p-6 rounded-lg shadow-lg w-11/12 sm:w-1/3">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <label for="logoutModalToggle"
                class="bg-gray-300 text-black text-xs h-full px-4 py-2 rounded-sm cursor-pointer">Cancel</label>

            <!-- Form submit button inside modal -->
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="bg-red-500 text-xs h-full text-white px-4 py-2 rounded-sm">Logout</button>
            </form>
